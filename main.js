...
function listenForUserInput() {
  ...
  tasksOnPage.forEach(task => {
    task.addEventListener("keyup", e => {
      if (e.key !== "Enter" || emptyField.test(task.value)) return // ignore if no task is being created or edited
      if (typeToDelete.test(task.value)) return deleteItem(task) // respond to a 'delete' slash command
      if (typeToComplete.test(task.value)) return tickItem(task) // respond to a 'done' slasb command
      editItem(task)
    })
  })
...
// editing page display /////////////////////////
/////// changes to the look of our app /////////
